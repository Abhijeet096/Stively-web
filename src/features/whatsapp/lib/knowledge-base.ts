...
export const REFUND_POLICY = `Stively's refund policy is delivery-based, not a blanket "refund anytime" guarantee:
- For a course or digital product (GenAI course, digital store products): non-refundable once any part has been accessed - the file downloaded, or any lesson/video/module opened. Since access is granted automatically and immediately after payment, this means non-refundable in practice from the moment of purchase, not after a grace period.
- The only exception for courses/digital products: if you were charged in error, charged twice for the same purchase, or a genuine technical issue on Stively's end prevented you from ever accessing what you paid for, contact team@stively.com within 7 days of payment for review - never promise this will be approved, only that it will be reviewed.
