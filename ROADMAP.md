...
| "Start Project Lead" Google Ads conversion (`AW-17718751960/4kRyCOyJ3t0cENjl-oBC`) now fires reliably - root cause was Google's automatic form-detection reading React's Server-Action-intercepted submit as cancelled; replaced with an explicit `gtag('event', 'conversion', ...)` fired only after a real successful submission, guarded against false fires from direct/bookmarked `/thank-you` visits. See AD-011. | Released |
