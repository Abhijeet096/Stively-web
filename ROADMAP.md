...
| WhatsApp + AI Sales System foundation (AD-021) - centralized, idempotent post-purchase service (`handleOrderPaid`) fixing two real bugs (guest checkout's duplicate "you paid" email; a webhook/verifyPayment race with no shared claim), minimal `WhatsAppContact`/`WhatsAppMessage` model promoting into `Lead`/`SalesLead` on qualification (no enum widening, no parallel CRM), server-only Meta Cloud API client, deterministic (non-AI) post-purchase cross-sell with its own delayed-cron path, inbound webhook with HMAC verification + message-id dedup, Groq-grounded AI reply engine with a curated knowledge base and hard anti-hallucination rules, keyword-based agency-lead escalation, `/admin/whatsapp` conversation visibility. Built as an AD-007 freeze exception at the founder's explicit direction. Fully wired but genuinely inert - no WhatsApp message can send or receive until real Meta credentials + approved templates are supplied (see the founder's implementation report for the exact checklist). | Released |
