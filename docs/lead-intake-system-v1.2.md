...
**Version:** Lead Intake System Architecture v1.2
**Status:** Business Optimization Review Complete
...
v1.1 is the approved baseline for this revision — this is a business-optimization pass, not a redesign. Every change below exists because it directly increases revenue, improves conversion, reduces operational cost, improves reporting, or prevents losing leads (see the new Field Addition Principle in §1) — nothing here is a structural rework of v1.1's models. See **Version History** at the end for the complete list of what changed and why. Sections not listed there are unchanged from v1.1.

**Sections revised in v1.2:** §1 (Goals — new Field Addition Principle), §2 (Lead Lifecycle — Business `LOST` confirmed, no longer a flagged gap), §3 (Database Design — `lostReason`, `estimatedValue`, `nextFollowUpAt` added; Lead Age and Re-enquiry Count explicitly documented as computed, not stored), §5 (CRM Integration — new Lead Conversion Source Analytics subsection), §10 (Risks — one v1.1 open question resolved, assumptions from this revision added). New §11 (CEO Dashboard KPIs) added.
**Sections unchanged from v1.1:** §4 (Form Design), §6 (Automation — no new triggers, just new fields flowing through existing ones), §7 (API Design), §8 (Permissions), §9 (Future Expansion mechanism), the `TeamMember`, `LeadHistory`, `LeadAssignment`, and `LeadNote` models in §3.
...
### Field Addition Principle (new in v1.2)

Every field inside the Lead Intake System must satisfy at least one of: increase revenue, improve conversion, save time, improve reporting, or reduce human error. A field that doesn't satisfy at least one gets rejected, regardless of how easy it would be to add. This is the test every field added in this revision was held against (see §3's reasoning for each one), and it's meant to guide whoever touches this module next, not just this revision.

**Explicitly out of scope under this principle, restated from the brief so it's part of the durable record, not just a one-time instruction:** no AI, no machine learning, no predictive lead scoring, no automation builders, no email marketing platform, no workflow engine, no enterprise CRM features beyond what's documented here. Stively's Lead Intake stays lean and founder-first — sophistication gets added when a real operational need demonstrates it, not preemptively.

A retroactive check against v1.1's existing fields confirms none of them violate this principle (ownership → reduces human error and enables reporting; priority → improves conversion via better-prioritized follow-up; acquisition/UTM tracking → improves reporting) — stated here for completeness, not because anything needs to change.
...
(LOST reachable from any stage above Lead Created)
```
...
| `LOST` | **Confirmed in v1.2** — see below. Reachable from any stage; a project can fall through at Proposal just as easily as at first contact. |

**`LOST` confirmed in v1.2, resolving the v1.1 open question.** v1.1 flagged this pipeline's missing terminal failure state as a recommendation pending your decision (see the v1.1 entry in Version History). It's now an official status, with common reasons documented via `lostReason` (§3) — not a separate field per reason, one enum covering: budget issue, competitor won the business, an in-house team was chosen instead, the project was postponed, the project was cancelled, or no response after repeated follow-ups.

**Why this matters for reporting and conversion analytics, as asked:** without a `LOST` status, every Business lead that doesn't convert just sits in whatever stage it stalled at forever — `NEGOTIATION` or `PROPOSAL_SENT` leads that actually died six months ago look identical, in the data, to ones still genuinely in progress. That makes two things impossible: an honest conversion rate (the denominator is wrong if dead leads are still counted as "in the pipeline"), and any answer to "where in the funnel do we lose the most business leads" — which is exactly the kind of question §11's CEO Dashboard needs to answer and couldn't, before this.
...
model Lead {
  ...
  lostReason      LostReason?                        // NEW in v1.2

  leadType        LeadType
  metadata        Json?

  companyName     String?               // NEW in v1.1 - promoted out of metadata, see below

  priority        LeadPriority @default(MEDIUM)   // NEW in v1.1

  acquisitionChannel AcquisitionChannel?           // NEW in v1.1
  utmSource       String?                          // NEW in v1.1
  utmMedium       String?                          // NEW in v1.1
  utmCampaign     String?                           // NEW in v1.1

  estimatedValue  Int?                              // NEW in v1.2 - paise, Business leads only (see below)
  nextFollowUpAt  DateTime?                         // NEW in v1.2 - both lead types

  currentOwnerId  String?                           // NEW in v1.1 - denormalized pointer, see below
  ...
  lastEnquiryAt   DateTime    @default(now())       // NEW in v1.1 - see Deduplication below

  @@index([email])
  @@index([phone])
  @@index([nextFollowUpAt])                          // NEW in v1.2 - see below
...
**`lostReason` — new in v1.2, one enum, not dozens of fields.** Reconciles two reason lists given in the brief (one under "Business LOST Pipeline," one under "Lost Reason") into a single consistent set, rather than presenting them as two different things — flagging that reconciliation here since it's an interpretation, not a literal transcription of either list. Follows the exact same wide-enum pattern already established for `LeadStatus` (§3 above) rather than inventing a new one: Student-specific and Business-specific reasons share one enum, application-layer validation (not the database) ensures a lead's `leadType` and `lostReason` combination makes sense — same trade-off, same reasoning, already accepted once in this document. Nullable at the schema level (Prisma/Postgres can't cleanly express "required only when status is LOST or NOT_INTERESTED" declaratively without a check constraint, which is more machinery than this warrants) but should be enforced as required at the `submitLead`/status-update validation boundary whenever a status transition lands on `LOST` or `NOT_INTERESTED` — the same boundary-validation pattern used everywhere else in this document.

**Why `lostReason` increases business value, not just tidiness:** without it, a `LOST` status only tells you a deal died, never why — which makes it impossible to answer the single most useful question a founder can ask about a stalling pipeline: is the problem price, competition, timing, or fit? Those have completely different fixes (pricing page copy, competitive positioning, follow-up cadence, and qualification criteria, respectively), and a plain `LOST` count can't distinguish between them.

**`estimatedValue` — new in v1.2, one optional numeric field, exactly as instructed.** Stored as `Int` in paise, mirroring the exact convention `Program.price` already uses elsewhere in this schema — not a new money-representation pattern. Semantically meaningful for Business leads (an estimated project value); left null for Student leads, the same "field lives on the shared model but only some rows use it" pattern already established for `companyName`. This is what makes Total Pipeline Value, Expected Revenue, and Average Deal Size (§11) possible at all — none of those are computable without a number to sum or average somewhere.

**`nextFollowUpAt` — new in v1.2, indexed because it will be filtered on constantly.** Applies to both lead types, per the brief ("every lead should optionally have"). The `@@index([nextFollowUpAt])` addition is deliberate: "today's follow-ups," "overdue follow-ups," and "upcoming calls" (§11) are all range/comparison queries against this exact column (`nextFollowUpAt <= NOW()`, `nextFollowUpAt = TODAY`), and those need to stay fast as the leads table grows into the thousands — an unindexed date column is a classic, easy-to-miss source of a dashboard that works fine in testing and slows down months later once there's real data behind it.
...
**Why this matters for analytics and marketing ROI, as asked:** without this, there's no way to answer the Core Blueprint's own CEO Dashboard question (`docs/core/stively-core-blueprint-v1.md` §11: "Which ad generates highest ROI?") — knowing a lead came from `PROGRAM_INTEREST` (which page) says nothing about whether it came from an organic search, a paid Instagram ad, or a referral link, and those have completely different costs behind them. UTM parameters are the industry-standard mechanism for this because they're set by the marketer at campaign-creation time (in the ad platform or the link itself) and require no extra work from the visitor — the data arrives with the very first page load, before the lead even fills out a form. Without capturing it, cost-per-lead and cost-per-conversion can never be broken down by channel or campaign, which is the specific number that justifies (or kills) a given marketing spend. See this document's own §11 (new in v1.2) for the full KPI list this enables.

### Lead Age (new in v1.2 — no database column)

Lead Age = `CURRENT_DATE - Lead.createdAt`, computed at query time, never stored.

**Why no column, as explicitly asked:** a stored `leadAge` column would be wrong the moment a day passes — either it goes stale immediately (nobody's re-writing every row in the table every night just to keep an integer accurate), or it requires a scheduled job to update every single `Lead` row daily purely to keep a value fresh that Postgres can compute correctly, for free, at the exact moment it's actually needed: `SELECT *, (NOW() - "createdAt") AS lead_age FROM "Lead"`. Storing a value that's a pure function of another already-stored value (`createdAt`) is redundant by definition — the moment the derived value could be wrong, it's not doing its job, and the only way to keep it always right is to never store it in the first place.

**Purpose:** finding stale leads (`WHERE status NOT IN ('CONVERTED','LOST','NOT_INTERESTED') AND createdAt < NOW() - INTERVAL '7 days'`), ignored leads, and slow-moving pipelines — all queries the schema already supports today, with zero new fields.

### Re-enquiry Count (new in v1.2 — no database column)

Re-enquiry Count = `COUNT(LeadHistory WHERE leadId = X AND eventType = 'RE_ENQUIRY')` — also computed, also not stored, for the same reason as Lead Age: it's fully derivable from data this schema already captures.

**This one is effectively free.** v1.1's `LeadHistory`/`RE_ENQUIRY` design (built for the deduplication requirement, not for this) already logs every repeat enquiry as its own event — this section is purely a documentation note pointing out that a metric the brief flags as "may become useful later" already has zero-cost support in the existing design, not a new capability being built for it.

**Purpose, as given:** identifying highly-interested people who enquire multiple times — a real, if speculative, signal worth having queryable later, without having built anything specifically to enable it now.
...
**Revised in v1.2** — added a Lead Conversion Source Analytics subsection (below); everything else in this section is unchanged from v1.1.
...
### Lead Conversion Source Analytics (new in v1.2 — documentation only, no dashboard built)

How each requested metric would be computed from data this schema already captures, once something reads it:

| Metric | How it's derived |
|---|---|
| **Conversion Rate by Source** | `COUNT(status = CONVERTED) / COUNT(*)`, grouped by `acquisitionChannel` |
| **Revenue by Source** | `SUM(estimatedValue)` for `CONVERTED` Business leads, grouped by `acquisitionChannel` — an estimate, not actual invoiced revenue, since no Finance module exists yet to record real payment amounts against a Business lead |
| **Best Performing Campaign** | Same shape as the two rows above, grouped by `utmCampaign` instead of `acquisitionChannel` |
| **Cost Per Lead** | `(external ad spend for a channel) / COUNT(leads WHERE acquisitionChannel = X)` |
| **Cost Per Conversion** | `(external ad spend for a channel) / COUNT(CONVERTED leads WHERE acquisitionChannel = X)` |

**Cost Per Lead and Cost Per Conversion are flagged separately, deliberately.** The first three metrics are fully computable from this schema alone. The last two are not — they need actual marketing spend figures (how much was spent on Google Ads this month), which is Finance/marketing-budget data that doesn't exist anywhere in the Lead Intake System and isn't in this module's scope to hold. This document can define the formula; it cannot make the metric real without a spend-tracking source (a future Finance module per the blueprint, or in the near term, a manually-maintained spend figure per channel) to join against. Stating this plainly rather than implying the schema alone answers a question it can't yet.
...
**One of two v1.1 open pipeline questions is now resolved; one remains.** §2 v1.1 flagged two unanswered questions: whether the Business pipeline needs an explicit `LOST` status, and whether `NOT_RESPONDED` allows retry attempts. This revision resolves the first (confirmed, §2) but the second is still genuinely open — nothing in this revision's brief addressed it. Recommending it be answered before implementation begins, same as v1.1 said; not this document's call to make unilaterally.
...
**`lostReason`'s value list is a reconciliation, not a literal transcription — new risk in v1.2.** §3 explicitly notes this: the brief gave two overlapping-but-different reason lists (one under "Business LOST Pipeline," one under "Lost Reason"), and this document merged them into one consistent set. If the actual intent was two distinct, separate lists for two different purposes, this reconciliation is a wrong assumption that should be corrected before implementation, not discovered after.

**`estimatedValue`-derived revenue metrics are estimates, not actuals — new risk in v1.2.** "Revenue by Source" and "Average Deal Size" (§5/§11) are computed from `estimatedValue`, a number entered by a salesperson at some point in the pipeline, not from actual invoiced or collected payment. Presenting these as if they were real revenue in a future CEO Dashboard would be misleading — they should be labeled as estimates (e.g., "Estimated Pipeline Value," not "Revenue") until a real Finance module tracks actual payment amounts. Flagging this now so the distinction doesn't get lost between this document and however a future dashboard chooses to label these numbers.

---

## 11. CEO Dashboard KPIs (new in v1.2 — documentation only, no dashboard built)

Every KPI named in the brief, and where it comes from. Nothing here is a query or a UI — just confirming each KPI is actually answerable from the model this document defines, and flagging the two that aren't fully answerable yet.

| KPI | Data source |
|---|---|
| Student Leads / Business Leads | `COUNT(*) GROUP BY leadType` |
| New Leads Today | `COUNT(*) WHERE createdAt >= today` |
| Conversion Rate | `COUNT(status = CONVERTED) / COUNT(*)`, overall or `GROUP BY leadType` |
| Lost Rate | `COUNT(status IN (LOST, NOT_INTERESTED)) / COUNT(*)` |
| Pipeline Value | `SUM(estimatedValue) WHERE status NOT IN (CONVERTED, LOST, NOT_INTERESTED)` — see the estimate-vs-actual risk above |
| Revenue | Same caveat as Pipeline Value — this schema can only estimate revenue (`SUM(estimatedValue) WHERE status = CONVERTED`), not report actual collected revenue, until a Finance module exists |
| Average Sales Cycle | `AVG(LeadHistory.createdAt WHERE eventType = CONVERTED − Lead.createdAt)` — already fully supported by v1.1's `LeadHistory` design, no new field needed |
| Average Project Value | `AVG(estimatedValue) WHERE leadType = BUSINESS AND status = CONVERTED` |
| Top Acquisition Channel | See §5's Lead Conversion Source Analytics — this KPI *is* that section's output, not a separate calculation |
| Top Salesperson | `COUNT(CONVERTED) GROUP BY owner` — **implementation caveat, not just a data source note:** this should be computed from whichever `TeamMember` held an active `LeadAssignment` *at the moment the `CONVERTED` `LeadHistory` event fired*, not from `Lead.currentOwnerId` — a lead reassigned after converting would otherwise misattribute the close to whoever holds ownership now, not whoever actually closed it. |
| Pending Follow-ups | `COUNT(*) WHERE nextFollowUpAt <= NOW() AND status NOT IN (CONVERTED, LOST, NOT_INTERESTED)` |
...
**v1.2 — Business Optimization Review Complete. Implementation Pending.**
Baseline: v1.1, approved. This revision is additive/optimization-focused, not a redesign — no existing model was replaced, and every addition was checked against the new Field Addition Principle (§1).
- Added a Field Addition Principle to §1, formalizing "every field must increase revenue, improve conversion, save time, improve reporting, or reduce human error" as a standing rule for future contributors, and restated the explicit AI/ML/automation/enterprise-CRM exclusions as part of the durable record, not just a one-time instruction.
- **Confirmed `LOST` as an official Business pipeline status**, resolving the open question v1.1 explicitly flagged and left for this decision — §2.
- Added `lostReason` (one enum, Student and Business reasons grouped, following the same wide-enum pattern already established for `LeadStatus`) — §3. Flagged as a reconciliation of two overlapping lists given in the brief, not a literal transcription of either — §10.
- Added `estimatedValue` (one optional Int field, paise, mirroring `Program.price`'s existing convention) — §3. Flagged that revenue metrics derived from it are estimates, not actuals, until a Finance module exists — §10.
- Added `nextFollowUpAt` (one optional DateTime field, indexed) — §3.
- Documented Lead Age and Re-enquiry Count as computed-at-query-time values with no new database column, and explained why storing either would be actively wrong, not just unnecessary — §3.
- Added a Lead Conversion Source Analytics subsection to §5, defining five CEO-facing metrics against existing fields — explicitly separating the three fully computable from this schema alone from the two (Cost Per Lead, Cost Per Conversion) that need external ad-spend data this module doesn't and shouldn't hold.
- Added new §11 (CEO Dashboard KPIs) mapping every requested KPI to its data source, including one implementation caveat (Top Salesperson attribution) that a naive query would get wrong.
- Updated §10: resolved one v1.1 open question (Business `LOST`), added two new risks (the `lostReason` reconciliation assumption, and estimate-vs-actual revenue framing).
- **Deferred to a future CRM v2, not built or designed here:** any actual dashboard UI or query implementation (explicitly out of scope per the brief for every KPI/metric section above); automated stale-lead alerts based on Lead Age; SLA-breach detection logic (the schema supports recording *that* a reassignment happened for this reason, not *detecting* when an SLA has been breached); Re-enquiry Count surfaced as an actual feature or alert, rather than just a derivable metric; any actual ad-spend integration to make Cost Per Lead/Conversion real; AI, machine learning, predictive lead scoring, automation builders, email marketing, and workflow engines, per the brief's explicit exclusions, now also recorded in §1's Field Addition Principle.
